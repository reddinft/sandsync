...
Return ONLY valid JSON:
{
  ...
  "quality_score": 8.5,
  "approved": true,
  "rejection_reason": ["specific issue 1 that Anansi must fix", "specific issue 2"]
}

Score below 7.5 means the draft needs revision. Be honest and rigorous.
If rejecting (score < 7.5), list specific, actionable issues in rejection_reason that Anansi must fix in the next draft — not generic statements like "Quality below threshold" but precise problems like "The Soucouyant is described as a generic vampire — she must be portrayed as an elderly village woman who sheds her skin at night" or "The Caribbean dialect feels forced — remove the anglicised phonetics and use natural speech patterns instead".`;
        ...
        const attemptRecord = {
          ...
          rejection_reason:
            qualityScore < QUALITY_THRESHOLD
              ? [...(ogmaParsed?.rejection_reason || []), ...(ogmaParsed?.changes_made || [])].filter(Boolean) || ["Quality below threshold"]
              : undefined,
          cultural_notes: ogmaParsed?.cultural_notes || undefined,
        };
        ...
        await writeAgentEvent(supabase, storyId, "ogma", "completed", {
          ...
          rejection_reason: ogmaParsed?.rejection_reason || [],
          changes_made: ogmaParsed?.changes_made || [],
          cultural_notes: ogmaParsed?.cultural_notes || "",
        });
...
Ogma reviewed your draft and scored it ${qualityScore.toFixed(1)}/10. Here is what must be improved:
${[...(ogmaParsed?.rejection_reason || []), ...(ogmaParsed?.changes_made || [])].map((c: string) => `- ${c}`).join("\n")}

Cultural notes: ${ogmaParsed?.cultural_notes || "Improve cultural authenticity."}

Ogma's suggested revision: """
${ogmaParsed?.reviewed_content ? ogmaParsed.reviewed_content.slice(0, 500) + "..." : "See notes above"}
"""

Write a significantly improved version addressing ALL the above points. Score must reach 7.5+.
